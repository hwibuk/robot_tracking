...
import os
import sys
from geometry_msgs.msg import Twist  # Pose2D 삭제
...
class SerialBridgeNode(Node):
    def __init__(self):
        ...
        # Pose2D 대신 Twist 사용
        self.sub_auto = self.create_subscription(Twist, target_action, self.auto_action_callback, 10)
    ...
    def auto_action_callback(self, msg):
        try:
            # Pose2D의 x와 theta 대신 Twist의 linear.x와 angular.z 사용
            dist_val = msg.linear.x
            angle_val = msg.angular.z
            ...
            if dist_val == 0.0:
                ...
                # 아두이노가 readStringUntil('\n')로 읽을 수 있도록 끝에 \n 추가
                command = f"D{dist_val:.2f},A{angle_val:.2f},T{timestamp}\n"

...
from ament_index_python.packages import get_package_share_directory # 경로 탐색 라이브러리
...
class ArucoDetector(Node):
    ...
    def load_params(self):
        try:
            # 패키지 설치 경로를 자동으로 찾도록 수정
            package_name = 'tracking_pj'
            package_share_directory = get_package_share_directory(package_name)
            path = os.path.join(package_share_directory, 'calibration_data.npz')
            ...
            self.get_logger().info(f"Loaded calibration data from: {path}")

        except Exception as e:
            self.get_logger().error(f"Failed to load params: {str(e)}")
            self.mtx = np.eye(3)
            self.dist = np.zeros(5)

...
class RobotController(Node):
    def __init__(self):
        ...
        # --- [통신 설정] ---
        # 입력을 받는 부분은 ArUco 노드의 출력에 맞춰 Pose2D를 유지해야 할 수도 있으나,
        # 요청에 따라 내부 통신 인터페이스를 Twist로 통일합니다.
        from geometry_msgs.msg import Pose2D
        self.sub_pose = self.create_subscription(Pose2D, '/target/relative_pose', self.pose_callback, 10)
        self.pub_action = self.create_publisher(Twist, '/robot_1/action', 10)
    ...
    def pose_callback(self, msg):
        ...
        if now - self.last_sent_time >= self.SEND_INTERVAL:
            ...
            if dist < self.STOP_DISTANCE:
                ...
                cmd.linear.x = dist
                cmd.angular.z = -angle if self.REVERSE_STEER else angle
